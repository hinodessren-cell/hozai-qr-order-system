  ...
  return (
    <div className={`app density-${settings.density}${ipadDevice && settings.ipadFullscreen ? " ipadFullscreen" : ""}`} style={{ "--accent": settings.accent } as React.CSSProperties}>
      ...
      {selectedItem && <OrderModal item={selectedItem} history={orders.filter((order) => order.id === selectedItem.id)} close={() => setSelectedItem(null)} submit={async (updatedItem, quantity, purchaser, orderNote) => {
        if (updatedItem.code !== selectedItem.code || updatedItem.name !== selectedItem.name || updatedItem.orderPoint !== selectedItem.orderPoint) await updateBoardItem(updatedItem);
