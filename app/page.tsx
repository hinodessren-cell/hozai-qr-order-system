  ...
  return (
    <div className={`app density-${settings.density}${ipadDevice && settings.ipadFullscreen ? " ipadFullscreen" : ""}`} style={{ "--accent": settings.accent } as React.CSSProperties}>
      ...
      <main>
        ...
        {tab === "history" && <HistoryOrderList key={query} orders={filtered} onEdit={setEditingOrder} onAdvance={advance} onCancel={cancelOrder} onDelete={deleteOrderHistory} onReturn={returnToWaiting} onReturnToOrdered={returnToOrdered} showMemo={settings.showMemo} />}
...
function HistoryOrderList(props: Omit<React.ComponentProps<typeof OrderList>, "title" | "onDelete"> & { onDelete: (id: string) => void }) {
  const pageSize = 30;
