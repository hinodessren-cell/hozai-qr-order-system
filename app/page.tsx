  ...
  return (
    <div className={`app density-${settings.density}${ipadDevice && settings.ipadFullscreen ? " ipadFullscreen" : ""}`} style={{ "--accent": settings.accent } as React.CSSProperties}>
      ...
      <main>
        ...
        {tab === "partialPrint" && <PartialPrintQueue items={printQueueItems} updateQueue={writePrintQueue} print={() => void printSelectedBoards(printQueueItems, true)} save={updateBoardItem} edit={setEditingItem} order={setSelectedItem} columns={settings.cardColumns} />}
...
function PartialPrintQueue({ items, updateQueue, print, save, edit, order, columns }: { items: Item[]; updateQueue: (ids: string[]) => void; print: () => void; save: (item: Item) => Promise<void>; edit: (item: Item) => void; order: (item: Item) => void; columns: number }) {
